...
export default function Side({ hall, toggleHall }: SideProps): JSX.Element {
  ...
  switch (hall) {
    case HallEnum.ANTEATERY:
      ...
      heroImageAlt =
        "An image of the front of the Anteatery dining hall at UCI.";
      break;
    case HallEnum.BRANDYWINE:
      heroImageSrc = "/brandywine.webp";
      heroImageAlt =
        "An image of the front of the Brandywine dining hall at UCI.";
      break;
  }
  ...
  if (!isLoading && !isError && hallData?.menus && hallData.menus.length > 0) {
    hallData.menus.forEach((menu) => {
      try {
        ...
        availablePeriodTimes[periodNameLower] = [
    ...
    else if (hallStatusDate >= openTime && hallStatusDate < closeTime)
  ...
  useEffect(() => {
    ...
    if (
      selectedDate &&
      periods.length > 0 &&
      Object.keys(availablePeriodTimes).length > 0
    ) {
      const currentPeriodIsValid = periods.some(
        (p) => p.toLowerCase() === selectedPeriod.toLowerCase(),
      );
  ...
  useEffect(() => {
    if (fetchedStations.length > 0) {
      ...
      const currentSelectionIsValid = fetchedStations.some(
    ...
    // NOTE: We only want to reset based on the *availability* of stations.
  }, [fetchedStations]); // Re-run when fetchedStations array identity changes
  ...
  return (
    <div className="z-0 flex flex-col h-full overflow-x-hidden">
      ...
      <div className="p-5 flex flex-col" id="side-content">
